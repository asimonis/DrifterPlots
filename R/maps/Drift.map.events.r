...
Drift.map.events <- function(outfilename, station.numbers = NULL, speciesID=NULL, MapDir=NULL,DBDir=NULL,spotcsvfile, DriftFile='Drift_FileLookup.csv',
    ...
    for(n in 1:n.stations){
      ...
      if(!is.na(lookup$dateTimeStart[n*2])){  # for DASBRS that have been deployed...
        ...
        if(!is.na(lookup$dateTimeEnd[LookupInd[1]])){ # for DASBRs that have been retrieved
          ...
          if(stations[n] %in% c(7,14,20)){offsetV<- -0.1}
