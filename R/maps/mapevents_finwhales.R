...
map.eventsBp <- function(outfilename, station.numbers = NULL, speciesID, MapDir=NULL,DBDir=NULL, 
  ...
  if(extent=="Bp") {lon1= -132;lon2= -114;lat1=27;lat2=45}
  if(extent=="Other"){
  ...
  for(S in 1:length(speciesID)){
    ...
    plot.bathy(bat,image=TRUE,bpal=list(c(0,max(bat),greys),c(min(bat),0,blues)),land=TRUE,n=0, main=figtitle, 
               deepest.isobath=-500, shallowest.isobath=-500,col='grey32',asp=NA)
    
    scaleBathy(bat, deg=1.01, x="bottomleft", inset=5)
    ...
    for(n in 1:n.stations){
      ...
      offsetV<-0.4
      offsetH<-0
      if(station.numbers[n] %in% c(7)){offsetH<- -0.4}
      ...
      Events <- dbReadTable(conn, "Fin_Whale_20_Hz_Detector_OfflineEvents")         #read offline events
      dbDisconnect(conn)

      Events<-filter(Events,nClicks>0)
      ...
      ColInd<-1
       points(Events$long, Events$lat, col=SpColor, pch=19,cex=.2)
      
    }
    
    legend('topright', legend='Fin whale', pch=19,cex=1,pt.cex = 1, col=SpColor[which(SPLabels == speciesID[[S]][1]):ColInd], bg="white")
